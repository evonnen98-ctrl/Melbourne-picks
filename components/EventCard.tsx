...
function googleHref(name: string): string {
  return `https://www.google.com/search?q=${encodeURIComponent(name + ' Melbourne 2025')}`
}

export default function EventCard({ place, index, onSave, isSaved, isCloseMatch }: EventCardProps) {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-zinc-100/80 hover:shadow-md transition-shadow duration-200">
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <button
          onClick={handleSave}
          aria-label={isSaved ? 'Saved' : 'Save this event'}
      ...
      <div className="flex gap-2">
        <a
          ...
          className={`text-center text-xs font-medium py-2.5 rounded-xl border border-zinc-200 text-zinc-500 hover:border-zinc-400 hover:text-zinc-700 transition-all duration-150 active:scale-[0.98] ${place.ticketUrl ? 'flex-1' : 'w-full'}`}
        >
          Find out more
        </a>
